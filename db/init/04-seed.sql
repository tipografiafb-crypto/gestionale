...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM stores) = 0 THEN
    ...
    PERFORM setval('stores_id_seq', 5);
  END IF;
  ...
  IF (SELECT COUNT(*) FROM product_categories) = 0 THEN
    INSERT INTO product_categories (id, name, description, active) VALUES
      (1, 'Plettri', '', true);
    PERFORM setval('product_categories_id_seq', 1);
  END IF;
  ...
  IF (SELECT COUNT(*) FROM switch_webhooks) = 0 THEN
    ...
    PERFORM setval('switch_webhooks_id_seq', 1);
  END IF;
  ...
  IF (SELECT COUNT(*) FROM print_flows) = 0 THEN
    ...
    PERFORM setval('print_flows_id_seq', 2);
  END IF;
  ...
  IF (SELECT COUNT(*) FROM print_machines) = 0 THEN
    INSERT INTO print_machines (id, name, description, active) VALUES
      (1, 'signracer', '', true);
    PERFORM setval('print_machines_id_seq', 1);
  END IF;
  ...
  IF (SELECT COUNT(*) FROM products) = 0 THEN
    ...
    PERFORM setval('products_id_seq', 7);
  END IF;

  -- Insert inventory records
  IF (SELECT COUNT(*) FROM inventories) = 0 THEN
    INSERT INTO inventories (product_id, quantity_in_stock) VALUES
      (7, 100);
    PERFORM setval('inventories_id_seq', 1);
  END IF;
